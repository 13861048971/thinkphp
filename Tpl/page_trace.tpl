...
<div id="think_page_trace_tab_tit">
    ...
    <span class="tab_tit"><?php echo $key ?></span>
    <?php } ?>
</div>
<div id="think_page_trace_tab_cont" style="overflow:auto;height:212px;padding: 0; line-height: 24px">
	...
	if(is_array($info)){
		...
		echo '<li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px"><pre>' . (is_numeric($k) ? '' : $k.' : ') . htmlentities($val,ENT_COMPAT,'utf-8') .'</pre></li>';
	    }
	}
    ?>
    </ol>
	...
	tab_tit[i].onclick = (function(i){
		return function(){
			for(var j = 0; j < tab_cont.length; j++){
				...
				tab_tit[j].className = 'tab_tit';
			}
			tab_cont[i].style.display = 'block';
			tab_tit[i].className = 'tab_tit cur';
			history[1] = i;
			document.cookie = 'thinkphp_show_page_trace='+history.join('|')
		}
	})(i)
...
<style>
#think_page_trace_tab_tit{padding:0 0 0 10px; background:#eee;}
.tab_tit{ color:#000;padding:0px 10px;height:33px;line-height: 33px;
			cursor: pointer;font-weight:700;display:inline-block;color:#999; border:1px solid #eee; border-bottom:none;}
.tab_tit.cur{  border:1px solid #ccc; border-top:2px solid #16A085; margin:-1px 0 0 0;
			border-bottom:0px solid #fff ; color:#000; background:#fff; height:35px;}
#think_page_trace_tab_tit{ height:35px;border-bottom:1px solid #ccc;border-top:1px solid #ccc;
